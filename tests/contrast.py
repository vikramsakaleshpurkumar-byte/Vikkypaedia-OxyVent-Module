...
sel = ['#continueBtn','#expandAll','#collapseAll','.toggle[aria-pressed="true"]','.toggle[aria-pressed="false"]',
       ...
       '.hint h6','.lede','.hc-note','.side .part-label','.lk','footer p','.crit .cnote','.legend','.pb-id > .pb-avatar','.pb-id > div > b','.pb-id > div > span','.pb-plan','.onb-step h2','.onb-kicker','.onb-list li','.onb-note','.plan-tile .pv','.plan-tile .pl','.onb-skip','.sigfield > label','.sigmeta','.fig-t','.fig-alt > summary','.srch-foot','.srch-keys kbd','.nextstep .ns-k','#nsTitle','#nsDesc','.stat .sn','#dayWeek','.conf .conf-l','.conf button[aria-pressed="false"]','.conf button[aria-pressed="true"]','.cw-flag','.cw-flag b','#dueBtn','.printbtn','.box.roles h5','.box.roles li','.box.roles .roles-note']
