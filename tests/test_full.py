...
SKIP_ONB = "try{var K='vkp.oxyvent.v4';var r=localStorage.getItem(K);"\
...
with sync_playwright() as pw:
    ...
    print("\n== 2. MASTERY REVERSAL ==")
    seed_and_reload(p,
      """()=>{const s=JSON.parse(localStorage.getItem('vkp.oxyvent.v4'));
        s.items['1.1'].ok=false; s.items['1.1'].box=0;
        localStorage.setItem('vkp.oxyvent.v4',JSON.stringify(s));}""",
    ...
    seed_and_reload(p,
      """()=>{const K='vkp.oxyvent.v4';const s=JSON.parse(localStorage.getItem(K));
    ...
    check("attempt recorded", p.evaluate("()=>JSON.parse(localStorage.getItem('vkp.oxyvent.v4')).exam.attempts.length===1"))
    check("24h lock applied", p.evaluate("()=>JSON.parse(localStorage.getItem('vkp.oxyvent.v4')).exam.lockUntil>Date.now()"))
    ...
    check("verification code present", "OV26-" in p.evaluate("()=>document.getElementById('printRoot').textContent"))
    check("scope statement in print sheet", "does not attest bedside competence" in p.evaluate("()=>document.getElementById('printRoot').textContent"))
