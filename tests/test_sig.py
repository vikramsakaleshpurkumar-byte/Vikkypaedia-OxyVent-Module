...
SKIP=("try{var K='vkp.oxyvent.v4';var r=localStorage.getItem(K);"
...
with sync_playwright() as pw:
    ...
    st=p.evaluate("""()=>{const s=JSON.parse(localStorage.getItem('vkp.oxyvent.v4'));
    ...
    alpha=p.evaluate("""async()=>{const s=JSON.parse(localStorage.getItem('vkp.oxyvent.v4'));
    ...
    check("cleared from storage", p.evaluate("()=>{const s=JSON.parse(localStorage.getItem('vkp.oxyvent.v4'));return !(s.settings&&s.settings.signPhoto);}"))
